...
class User < ApplicationRecord
    ...
    def self.get_all_users
        self.all.select(:id, :name, :document, :age, :email, :password_digest)
    end

    # Query that gets the user corresponding to the id
    def self.get_user_by_id(current_id)
        self.where(id: current_id).select(:id, :name, :document, :age, :email, :password_digest).first
    end

    # Query that gets the user corresponding to the email
    def self.get_user_by_email(current_email)
        self.where(email: current_email).select(:id, :name, :document, :age, :email, :password_digest).first
    end

    # Query that updates an existing user record in DB
    def self.update_user(user_params, id_user)
        user = self.where(id: id_user).select(:id, :name, :document, :age, :email, :password_digest).first
